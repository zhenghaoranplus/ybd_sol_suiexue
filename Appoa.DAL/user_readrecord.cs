...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_user_readrecord"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_user_readrecord");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_user_readrecord(
                group_id            
                            ,data_id            
                            ,user_id            
                            ,add_time            
            ...
            parameters[1].Value=model.data_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_user_readrecord
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_user_readrecord SET 
                group_id = @group_id
                ,data_id = @data_id
                ,user_id = @user_id
                ,add_time = @add_time
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.user_readrecord.ALL_FILED + " from ybd_user_readrecord ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.user_readrecord.ALL_FILED + " FROM ybd_user_readrecord ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.user_readrecord.ALL_FILED);
            strSql.Append(" FROM ybd_user_readrecord ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.user_readrecord.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_user_readrecord T ");
