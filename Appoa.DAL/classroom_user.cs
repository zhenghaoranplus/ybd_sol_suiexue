...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_classroom_user"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_classroom_user");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.classroom_user model)
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_classroom_user(
                classroom_id            
                            ,user_id            
                            ,contents            
                            ,status            
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_classroom_user
            WHERE id = @id");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.classroom_user model)
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_classroom_user SET 
            ...
            parameters[1].Value=model.classroom_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.classroom_user.ALL_FILED + " from ybd_classroom_user ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.classroom_user.ALL_FILED + " FROM ybd_classroom_user ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.classroom_user.ALL_FILED);
            strSql.Append(" FROM ybd_classroom_user ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.classroom_user.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_classroom_user T ");
