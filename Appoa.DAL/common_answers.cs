...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_common_answers"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_common_answers");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_common_answers(
                question_id            
                            ,options            
                            ,contents            
                            ,score            
            ...
            SqlParameter[] parameters = 
			{
                     ...
                     ,new SqlParameter("@options",SqlDbType.NVarChar,50)
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_common_answers
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_common_answers SET 
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.common_answers.ALL_FILED + " from ybd_common_answers ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.common_answers.ALL_FILED + " FROM ybd_common_answers ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.common_answers.ALL_FILED);
            strSql.Append(" FROM ybd_common_answers ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.common_answers.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_common_answers T ");
