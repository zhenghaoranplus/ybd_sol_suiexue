...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_common_examination"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_common_examination");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_common_examination(
                group_id            
                            ,name            
                            ,parent_id            
                            ,nums            
            ...
            SqlParameter[] parameters = 
			{
                     ...
                     ,new SqlParameter("@descript",SqlDbType.NVarChar,4000)
                     ,new SqlParameter("@qrcode",SqlDbType.NVarChar,255)
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_common_examination
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_common_examination SET 
                group_id = @group_id
                ,name = @name
                ,parent_id = @parent_id
                ,nums = @nums
            ...
            parameters[8].Value=model.qrcode;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.common_examination.ALL_FILED + " from ybd_common_examination ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.common_examination.ALL_FILED + " FROM ybd_common_examination ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.common_examination.ALL_FILED);
            strSql.Append(" FROM ybd_common_examination ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.common_examination.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_common_examination T ");
