...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_user_cart"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_user_cart");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_user_cart(
                ct_id            
                            ,group_id            
                            ,user_id            
                            ,goods_sale_type            
            ...
            SqlParameter[] parameters = 
			{
                     ...
                     ,new SqlParameter("@goods_sale_type",SqlDbType.Int,4)
                     ,new SqlParameter("@goods_id",SqlDbType.Int,4)
                     ,new SqlParameter("@goods_name",SqlDbType.NVarChar,200)
                     ,new SqlParameter("@goods_subtitle",SqlDbType.NVarChar,200)
                     ,new SqlParameter("@goods_img",SqlDbType.NVarChar,255)
                     ...
                     ,new SqlParameter("@goods_spec_id",SqlDbType.NVarChar,255)
            ...
            parameters[3].Value=model.goods_sale_type;
            parameters[4].Value=model.goods_id;
            parameters[5].Value=model.goods_name;
            parameters[6].Value=model.goods_subtitle;
            parameters[7].Value=model.goods_img;
            parameters[8].Value=model.goods_oprice;
            parameters[9].Value=model.goods_price;
            parameters[10].Value=model.goods_spec;
            parameters[11].Value=model.goods_spec_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_user_cart
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_user_cart SET 
                ct_id = @ct_id
                ,group_id = @group_id
                ,user_id = @user_id
            ...
            parameters[4].Value=model.goods_sale_type;
            parameters[5].Value=model.goods_id;
            parameters[6].Value=model.goods_name;
            parameters[7].Value=model.goods_subtitle;
            parameters[8].Value=model.goods_img;
            parameters[9].Value=model.goods_oprice;
            parameters[10].Value=model.goods_price;
            parameters[11].Value=model.goods_spec;
            parameters[12].Value=model.goods_spec_id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.user_cart.ALL_FILED + " from ybd_user_cart ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.user_cart.ALL_FILED + " FROM ybd_user_cart ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.user_cart.ALL_FILED);
            strSql.Append(" FROM ybd_user_cart ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.user_cart.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_user_cart T ");
