...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_user_integral"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_user_integral");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_user_integral(
                user_id            
                            ,integral            
                            ,get_or_use            
                            ,type            
            ...
            SqlParameter[] parameters = 
			{
                     ...
                     ,new SqlParameter("@get_or_use",SqlDbType.Int,4)
                     ,new SqlParameter("@type",SqlDbType.Int,4)
                     ,new SqlParameter("@type_name",SqlDbType.NVarChar,50)
                     ,new SqlParameter("@add_time",SqlDbType.DateTime,8)
            };
            parameters[0].Value=model.user_id;
            parameters[1].Value=model.integral;
            parameters[2].Value=model.get_or_use;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_user_integral
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_user_integral SET 
                user_id = @user_id
                ,integral = @integral
                ,get_or_use = @get_or_use
                ,type = @type
                ,type_name = @type_name
                ,add_time = @add_time
            WHERE id = @id");
            ...
            parameters[3].Value=model.get_or_use;
            parameters[4].Value=model.type;
            parameters[5].Value=model.type_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.user_integral.ALL_FILED + " from ybd_user_integral ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.user_integral.ALL_FILED + " FROM ybd_user_integral ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.user_integral.ALL_FILED);
            strSql.Append(" FROM ybd_user_integral ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.user_integral.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_user_integral T ");
