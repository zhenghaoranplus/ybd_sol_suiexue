...
namespace Appoa.DAL
{
    ...
    {		
		...
		public int GetMaxId()
		{
		    return DbHelperSQL.GetMaxID("ID", "ybd_user_signin"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ybd_user_signin");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"Insert Into ybd_user_signin(
                user_id            
                            ,add_time            
             ) Values (
                @user_id               
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"DELETE from ybd_user_signin
            WHERE id = @id");
		...
		{
			StringBuilder strSql = new StringBuilder();
			strSql.Append(@"UPDATE ybd_user_signin SET 
                user_id = @user_id
                ,add_time = @add_time
            WHERE id = @id");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT TOP 1 " + Model.user_signin.ALL_FILED + " from ybd_user_signin ");
            strSql.Append("WHERE id = @id");
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT " + Model.user_signin.ALL_FILED + " FROM ybd_user_signin ");
        ...
        public DataSet GetList(int Top, string whereStr, string filedOrder)
        {
            ...
            strSql.Append(Model.user_signin.ALL_FILED);
            strSql.Append(" FROM ybd_user_signin ");
            if(whereStr.Length > 0)
            {
                strSql.Append(" WHERE " + whereStr);
            }
        ...
        {            
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT " + Model.user_signin.ALL_FILED + " FROM ( ");
            ...
            strSql.Append(")AS Row, T.*  FROM ybd_user_signin T ");
