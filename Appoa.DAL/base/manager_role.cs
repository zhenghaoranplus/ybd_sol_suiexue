...
namespace Appoa.DAL
{
    ...
    public partial class manager_role
    {
        ...
        public bool Update(Model.manager_role model)
        {
            ...
            strSql4.Append("delete from " + databaseprefix + "manager_role_value where role_id=@role_id ");
            ...
            if (model.manager_role_values != null)
            {
                ...
                {
                    //删除已存在的权限记录
                    if (Utils.StrToInt(DbHelperSQL.GetSingle("select count(id) from " + databaseprefix + "manager_role_value where nav_name='" + modelt.nav_name + "'").ToString(), 0) > 0)
                    {
                        StringBuilder strSql2 = new StringBuilder();
                        strSql2.Append("delete from " + databaseprefix + "manager_role_value where nav_name=@nav_name ");
                        SqlParameter[] parameters2 = {
                                new SqlParameter("@nav_name", SqlDbType.NVarChar,100)};
                        parameters2[0].Value = modelt.nav_name;
