...
namespace Appoa.DAL
{
	...
	public partial class sys_navigation
	{
        ...
        public sys_navigation(string _databaseprefix)
        {
		...
		public bool Exists(int id)
		{
            ...
            strSql.Append("select count(1) from " + databaseprefix + "sys_navigation");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "sys_navigation");
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.sys_navigation GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,parent_id,channel_id,nav_type,name,title,sub_title,icon_url,link_url,sort_id,is_lock,remark,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "sys_navigation ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "sys_navigation");
            strSql.Append(" where name=@nav_name");
            SqlParameter[] parameters = {
					new SqlParameter("@nav_name", SqlDbType.NVarChar,50)};
            parameters[0].Value = nav_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "sys_navigation set " + strValue);
            strSql.Append(" where id=" + id);
            int rows = DbHelperSQL.ExecuteSql(strSql.ToString());
            if (rows > 0)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "sys_navigation set name=@new_name");
        ...
        public int Add(string parent_name, string nav_name, string title, string link_url, int sort_id, int channel_id, string action_type)
        {
            ...
            strSql1.Append("select top 1 id from " + databaseprefix + "sys_navigation");
            ...
            parameters[2].Value = EnumCollection.NavigationEnum.System.ToString();
