...
namespace Appoa.Common
{
    public class Utils
    {
        ...
        {
            return AppoaKeys.ASSEMBLY_VERSION;
        }
        #endregion
        ...
        /// <returns></returns>
        public static string FitlerSpecial(string str)
        {
            if (!string.IsNullOrEmpty(str))
            {
                byte[] byteStr = System.Text.Encoding.Default.GetBytes(str.ToString());
                for (int i = 0; i < byteStr.Length - 1; i++)
                {
                    if ((int)byteStr[i] == 32) byteStr[i] = (byte)43;//+号处理
                    if ((int)byteStr[i] == 10) byteStr[i] = (byte)13;//LF换行符处理
                }
                return System.Text.Encoding.Default.GetString(byteStr);
            }
            return "";
        }
        #endregion
        ...
        {
            if (expression != null)
                return Regex.IsMatch(expression.ToString(), @"^1[3|4|5|7|8]\d{9}$");
        ...
        public static void DeleteUpFile(string _filepath)
        {
            try
            {
                ...
                if (File.Exists(fullpath))
                {
                    if (File.GetAttributes(fullpath).ToString().IndexOf("ReadOnly") != -1)
                    {
            ...
            {
                StringToTxt("DeleteUpFile删除上传的文件时发生异常：" + e.TargetSite);
            }
        }
        ...
        public static void StringToTxt(string str)
        {
            ...
            string f = ws + "/logs/EXEClog" + DateTime.Now.ToString("yyyyMMdd") + ".txt";
            if (!System.IO.Directory.Exists(ws + "/logs/"))
            {
                Directory.CreateDirectory(ws + "/logs/");
            }
            if (!File.Exists(f))
            {
        ...
        #region 计算两点之间距离
        public static double GetDistance(double LngBegin, double LatBegin, double LngEnd, double LatEnd)
        {
            double Distance;
            double EARTH_RADIUS = 6378.137;
            double RadLatBegin, RadLatEnd, RadLatDiff, RadLngDiff;
            RadLatBegin = LatBegin * Math.PI / 180.0;
            RadLatEnd = LatEnd * Math.PI / 180.0;
            RadLatDiff = RadLatBegin - RadLatEnd;
            RadLngDiff = LngBegin * Math.PI / 180.0 - LngEnd * Math.PI / 180.0;
        ...
        public static string GetTimeNo()
        {
            lock (locker)
            {
                ...
                Thread.Sleep(10);
                return DateTime.Now.ToString("yyyyMMddHHmmssffff") + _no.ToString("000");
            }
        }
        #endregion
    }
