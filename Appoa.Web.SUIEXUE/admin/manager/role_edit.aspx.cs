...
namespace Appoa.Manage.admin.manager
{
    public partial class role_edit : Web.UI.ManagePage
    {
        ...
        private int id = 0;
        private Model.sys_manager manager = new Model.sys_manager();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_sys_role_list", EnumCollection.ActionEnum.View.ToString()); //检查权限
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(EnumCollection.ActionEnum.Add.ToString(), "添加管理角色:" + model.role_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "修改管理角色:" + model.role_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void rptList_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                ...
                for (int i = 0; i < actionTypeArr.Length; i++)
                {
                    ...
                    {
                        if (actionTypeArr[i] == "Show" || actionTypeArr[i] == "View" || actionTypeArr[i] == "Add" || actionTypeArr[i] == "Modify" || actionTypeArr[i] == "Delete")
                        {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ChkAdminLevel("_sys_role_list", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("_sys_role_list", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
