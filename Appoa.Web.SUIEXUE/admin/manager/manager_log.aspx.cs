...
namespace Appoa.Manage.admin.manager
{
    public partial class manager_log : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_sys_manager_log", EnumCollection.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("_sys_manager_log", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            BLL.manager_log bll = new BLL.manager_log();
            int sucCount = bll.Delete(7);
            AddAdminLog(EnumCollection.ActionEnum.Delete.ToString(), "删除管理日志" + sucCount + "条"); //记录日志
