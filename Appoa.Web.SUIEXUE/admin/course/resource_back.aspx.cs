...
namespace Appoa.Manage.admin.course
{
    public partial class resource_back : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_resource_back", EnumCollection.ActionEnum.View.ToString()); //检查权限
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("resource_back_page_size", "AppoaPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("resource_back_page_size", "AppoaPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("_resource_back", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.common_resource bll = new BLL.common_resource();
        ...
        {
            ChkAdminLevel("_resource_back", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.common_resource bll = new BLL.common_resource();
            for (int i = 0; i < rptList.Items.Count; i++)
