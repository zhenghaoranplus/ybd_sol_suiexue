...
namespace Appoa.Manage.admin.course
{
    public partial class chapter_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.course_id = RequestHelper.GetQueryInt("course_id");
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ...
                JscriptMsg("修改章节信息成功！", "chapter_list.aspx?course_id=" + course_id);
            }
            else //添加
            {
                ChkAdminLevel("_course_chapter", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加章节信息成功！", "chapter_list.aspx?course_id=" + course_id);
            }
        }
    }
}