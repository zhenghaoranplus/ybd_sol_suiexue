...
namespace Appoa.Manage.admin.settings
{
    public partial class sys_config : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_sys_config", EnumCollection.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("_sys_config", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
            BLL.siteconfig bll = new BLL.siteconfig();
            Model.siteconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "修改系统配置信息"); //记录日志
