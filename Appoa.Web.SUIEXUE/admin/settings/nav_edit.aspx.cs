...
namespace Appoa.Manage.admin.settings
{
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == EnumCollection.ActionEnum.Modify.ToString())
            {
                ...
                if (!new BLL.sys_navigation().Exists(this.id))
                {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_sys_navigation", EnumCollection.ActionEnum.View.ToString()); //检查权限
                TreeBind(EnumCollection.NavigationEnum.System.ToString()); //绑定导航菜单
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.nav_type = EnumCollection.NavigationEnum.System.ToString();
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(EnumCollection.ActionEnum.Add.ToString(), "添加导航菜单:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("_sys_navigation", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
