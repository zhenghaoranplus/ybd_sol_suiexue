...
{
    public partial class case_category : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_case_category", EnumCollection.ActionEnum.View.ToString()); //检查权限
                RptBind();
            }
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("case_category_page_size", "AppoaPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("case_category_page_size", "AppoaPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("case_category.aspx", "", ""));
        }

        //删除导航
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("_case_category", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            BLL.common_category bll = new BLL.common_category();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
            ...
            JscriptMsg("删除数据成功！", Utils.CombUrlTxt("case_category.aspx", "", ""));
        }
    }
}