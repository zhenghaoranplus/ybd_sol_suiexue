...
namespace Appoa.Manage.admin.pc
{
    public partial class about_us : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_pc_about_us", EnumCollection.ActionEnum.View.ToString()); //检查权限
        ...
        private void ShowInfo()
        {
            ...
            Model.common_article model = bll.GetModel(" group_id = " + (int)EnumCollection.article_group.PC站关于我们);
        ...
        private bool DoEdit(int id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "修改PC站关于我们，主键:" + id); //记录日志
                return true;
            }
            else
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (model != null)
            {
                ChkAdminLevel("_pc_about_us", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
                if (!DoEdit(model.id))
                {
                ...
                JscriptMsg("修改PC站关于我们成功！", "");
            }
            else
            {
                ChkAdminLevel("_pc_about_us", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
