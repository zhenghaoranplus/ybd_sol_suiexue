...
{
    public partial class case_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.group_id = (int)EnumCollection.article_group.客户案例;
            model.user_id = 0;
            ...
            if (id > 0)
            {
                AddAdminLog(EnumCollection.ActionEnum.Add.ToString(), "添加客户案例，主键:" + id); //记录日志
                return true;
            }
            else
            {
        ...
        private bool DoEdit(int id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "修改客户案例，主键:" + model.id); //记录日志
                return true;
            }
            else
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ...
                JscriptMsg("修改客户案例成功！", "case_list.aspx?page=" + this.page);
            }
            else //添加
            {
                ChkAdminLevel("_case_list", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加客户案例成功！", "case_list.aspx");
            }
        }
    }
}