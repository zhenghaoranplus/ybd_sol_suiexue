...
{
    public partial class case_category_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.group_id = (int)EnumCollection.category_group.客户案例;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ...
                JscriptMsg("修改客户案例分类成功！", "case_category.aspx");
            }
            else //添加
            {
                ChkAdminLevel("_case_category", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加客户案例分类成功！", "case_category.aspx");
            }
        }
    }
}