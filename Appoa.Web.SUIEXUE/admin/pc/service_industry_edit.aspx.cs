...
{
    public partial class service_industry_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.group_id = (int)EnumCollection.category_group.服务行业;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ...
                JscriptMsg("修改服务行业分类成功！", "service_industry.aspx");
            }
            else //添加
            {
                ChkAdminLevel("_service_industry", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加服务行业分类成功！", "service_industry.aspx");
            }
        }
    }
}