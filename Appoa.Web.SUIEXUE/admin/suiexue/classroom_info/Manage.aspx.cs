...
namespace Appoa.Manage.admin.classroom_info
{
    public partial class Manage : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_classroom_info", EnumCollection.ActionEnum.Manage.ToString()); //检查权限
                BindData();
            }
        }
        ...
        {
            ChkAdminLevel("_ybd_classroom_info", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.classroom_info bll = new BLL.classroom_info();
