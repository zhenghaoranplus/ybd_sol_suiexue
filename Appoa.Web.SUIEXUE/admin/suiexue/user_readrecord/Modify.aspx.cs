...
namespace Appoa.Manage.admin.user_readrecord
{
    public partial class Modify : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_user_readrecord", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
                BindInfo();
            }
        }
        ...
        private void BindInfo()
        {
            ...
            Model.user_readrecord model = bll.GetModel(this.id);
            if (model == null)
            {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "修改用户阅读记录信息，主键:" + id); //记录日志
                JscriptMsg("修改用户阅读记录信息成功！", "Manage.aspx");
            }
            else
            {
