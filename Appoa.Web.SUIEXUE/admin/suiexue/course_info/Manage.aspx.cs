...
namespace Appoa.Manage.admin.course_info
{
    public partial class Manage : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_course_info", EnumCollection.ActionEnum.Manage.ToString()); //检查权限
                BindData();
            }
        }
        ...
        {
            ChkAdminLevel("_ybd_course_info", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.course_info bll = new BLL.course_info();
