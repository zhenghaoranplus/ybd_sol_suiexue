...
namespace Appoa.Manage.admin.common_answers
{
    public partial class Manage : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_common_answers", EnumCollection.ActionEnum.Manage.ToString()); //检查权限
                BindData();
            }
        }
        ...
        {
            ChkAdminLevel("_ybd_common_answers", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.common_answers bll = new BLL.common_answers();
