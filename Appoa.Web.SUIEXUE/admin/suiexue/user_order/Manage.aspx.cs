...
namespace Appoa.Manage.admin.user_order
{
    public partial class Manage : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_user_order", EnumCollection.ActionEnum.Manage.ToString()); //检查权限
                BindData();
            }
        }
        ...
        {
            ChkAdminLevel("_ybd_user_order", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.user_order bll = new BLL.user_order();
