...
namespace Appoa.Manage.admin.common_resource
{
    public partial class Manage : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_ybd_common_resource", EnumCollection.ActionEnum.Manage.ToString()); //检查权限
                BindData();
            }
        }
        ...
        {
            ChkAdminLevel("_ybd_common_resource", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.common_resource bll = new BLL.common_resource();
