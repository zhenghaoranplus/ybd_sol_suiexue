$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "jpg,jpge,png,gif,apk", //文件类型
        filesize: "2048", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var uploader = WebUploader.create({
                ...
                fileSingleSizeLimit: p.filesize * 102400 //文件大小
            });
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        if (p.thumbnail) {
                            parentObj.siblings(".upload-path").val(data.thumb);
                            parentObj.siblings(".upload-imgs").attr('src', data.thumb);
                        } else {
                            parentObj.siblings(".upload-path").val(data.path);
                            parentObj.siblings(".upload-imgs").attr('src', data.path);
                        }
...
function addImage(targetObj, originalSrc, thumbSrc) {
    var hiobj = $(targetObj).attr('id');
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="' + hiobj + '_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    //+ '<input type="hidden" name="' + hiobj + '_remark" value="" />'
