...
{
    public partial class video_list : Web.UI.ManagePage
    {
        ...
        private void BindData()
        {
            ...
            string whereStr = " from_id = " + (int)EnumCollection.resource_from.课堂 + " and is_del = 0 and type = " + (int)EnumCollection.resource_type.视频资源 + " and data_id = " + this.class_id;
            ...
            if (!string.IsNullOrEmpty(_keywords))
            {
                ...
                {
                    JscriptMsg("搜索关键词中包含危险字符，检索终止！", Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}", "", this.class_id + ""));
                    return;
                }
            }
            ...
            string pageUrl = Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}&page={2}", this.keywords, this.class_id + "", "__id__");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("video_list_page_size", "AppoaPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}", txtKeywords.Text, this.class_id + ""));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("video_list_page_size", "AppoaPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}", this.keywords, this.class_id + ""));
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(EnumCollection.ActionEnum.Modify.ToString(), "保存课堂视频排序"); //记录日志
            JscriptMsg("保存排序成功！", Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}&page={2}", this.keywords, this.class_id + "", this.page + ""));
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("_classroom_video_materials", EnumCollection.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.common_resource bll = new BLL.common_resource();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}", this.keywords, this.class_id + ""));
        }

        protected void ddlddlClass_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(Utils.CombUrlTxt("video_list.aspx", "keywords={0}&class_id={1}", this.keywords, this.ddlClass.SelectedValue));
        }
