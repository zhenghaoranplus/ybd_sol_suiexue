...
namespace Appoa.Manage.admin.shop
{
    public partial class category_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.group_id = (int)EnumCollection.category_group.商城;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ...
                JscriptMsg("修改商品分类信息成功！", "category_list.aspx");
            }
            else //添加
            {
                ChkAdminLevel("_goods_category", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
                }
                JscriptMsg("添加商品分类信息成功！", "category_list.aspx");
            }
        }
    }
}