...
namespace Appoa.Manage.admin.shop
{
    public partial class adR_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("_shop_adR", EnumCollection.ActionEnum.View.ToString()); //检查权限
        ...
        private bool DoAdd()
        {
            ...
            model.ad_group_id = (int)EnumCollection.adR_group.商城轮播图;
            model.ad_group_name = EnumCollection.adR_group.商城轮播图.ToString();
            model.ad_type_id = 0;
            model.ad_type_name = "";
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == EnumCollection.ActionEnum.Modify.ToString()) //修改
            {
                ChkAdminLevel("_shop_adR", EnumCollection.ActionEnum.Modify.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("_shop_adR", EnumCollection.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "");
                    return;
