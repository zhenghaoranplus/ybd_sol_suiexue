...
namespace Appoa.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        /// 取得管理员信息
        /// </summary>
        public Model.sys_manager GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Model.sys_manager model = Session[AppoaKeys.SESSION_ADMIN_INFO] as Model.sys_manager;
                if (model != null)
                {
                    return model;
                }
        ...
        protected void JscriptDialog(string msgtitle, string msgcontent, string callback)
        {
            string msbox = "doSelect(\"" + msgtitle + "\", \"" + msgcontent + "\", " + callback + ")";
            ClientScript.RegisterClientScriptBlock(Page.GetType(), "doSelect", msbox, true);
        }
