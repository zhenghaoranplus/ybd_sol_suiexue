...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.user_integral dal = new DAL.user_integral();
        /// <summary>
        /// 无参构造函数
        /// </summary>
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.user_integral model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.user_integral model)
		{
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.user_integral GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.user_integral DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["get_or_use"] != null )//&& row["get_or_use"].ToString() != ""
                {
                    model.get_or_use = Convert.ToInt32(row["get_or_use"]);
                }
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.user_integral> DataTableToList(DataTable dt)
		{
			List<Model.user_integral> modelList = new List<Model.user_integral>();
            if (dt != null)
            {
				Model.user_integral model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.user_integral> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.user_integral GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.user_integral model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.user_integral> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.user_integral GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.user_integral> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.user_integral> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
