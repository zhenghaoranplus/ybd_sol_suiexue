...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.common_feedback dal = new DAL.common_feedback();
        /// <summary>
        /// 无参构造函数
        /// </summary>
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.common_feedback model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.common_feedback model)
		{
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.common_feedback GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.common_feedback DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                {
                    model.is_read = Convert.ToInt32(row["is_read"]);
                }
                if (row["read_time"] != null && row["read_time"].ToString() != "")
                {
                    model.read_time = Convert.ToDateTime(row["read_time"]);
                }
                return model;
            }
            else
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.common_feedback> DataTableToList(DataTable dt)
		{
			List<Model.common_feedback> modelList = new List<Model.common_feedback>();
            if (dt != null)
            {
				Model.common_feedback model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.common_feedback> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.common_feedback GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.common_feedback model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.common_feedback> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.common_feedback GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.common_feedback> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.common_feedback> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
