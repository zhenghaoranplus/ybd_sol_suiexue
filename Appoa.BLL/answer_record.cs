...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.answer_record dal = new DAL.answer_record();
        /// <summary>
        /// 无参构造函数
        /// </summary>
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.answer_record GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.answer_record DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["is_truth"] != null )//&& row["is_truth"].ToString() != ""
                {
                    model.is_truth = Convert.ToInt32(row["is_truth"]);
                }
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.answer_record> DataTableToList(DataTable dt)
		{
			List<Model.answer_record> modelList = new List<Model.answer_record>();
            if (dt != null)
            {
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.answer_record> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.answer_record GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.answer_record model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.answer_record> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.answer_record GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.answer_record> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.answer_record> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
