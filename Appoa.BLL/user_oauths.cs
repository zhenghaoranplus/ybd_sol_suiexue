...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.user_oauths dal = new DAL.user_oauths();
        /// <summary>
        /// 无参构造函数
        /// </summary>
        public user_oauths()
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.user_oauths model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.user_oauths model)
		{
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.user_oauths GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.user_oauths DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                {
                    model.appid = Convert.ToString(row["appid"]);
                }
                if (row["unionid"] != null )//&& row["unionid"].ToString() != ""
                {
                    model.unionid = Convert.ToString(row["unionid"]);
                }
                return model;
            }
            else
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.user_oauths> DataTableToList(DataTable dt)
		{
			List<Model.user_oauths> modelList = new List<Model.user_oauths>();
            if (dt != null)
            {
				Model.user_oauths model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.user_oauths> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.user_oauths GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.user_oauths model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.user_oauths> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.user_oauths GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.user_oauths> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.user_oauths> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
