...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.goods_spec_item dal = new DAL.goods_spec_item();
        /// <summary>
        /// 无参构造函数
        /// </summary>
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.goods_spec_item GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.goods_spec_item DataRowToModel(DataRow row)
        {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.goods_spec_item> DataTableToList(DataTable dt)
		{
			List<Model.goods_spec_item> modelList = new List<Model.goods_spec_item>();
            if (dt != null)
            {
				Model.goods_spec_item model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.goods_spec_item> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.goods_spec_item GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.goods_spec_item model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.goods_spec_item> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.goods_spec_item GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.goods_spec_item> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.goods_spec_item> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
