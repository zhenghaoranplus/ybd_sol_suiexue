...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.common_answers dal = new DAL.common_answers();
        /// <summary>
        /// 无参构造函数
        /// </summary>
		...
		/// 增加一条数据
		/// </summary>
		public int Add(Model.common_answers model)
		{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Model.common_answers model)
		{
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.common_answers GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.common_answers DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                {
                    model.question_id = Convert.ToInt32(row["question_id"]);
                }
                if (row["options"] != null )//&& row["options"].ToString() != ""
                {
                    model.options = Convert.ToString(row["options"]);
                }
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.common_answers> DataTableToList(DataTable dt)
		{
			List<Model.common_answers> modelList = new List<Model.common_answers>();
            if (dt != null)
            {
				Model.common_answers model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.common_answers> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.common_answers GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.common_answers model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.common_answers> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.common_answers GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.common_answers> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.common_answers> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
