...
namespace Appoa.BLL
{
    ...
    {
        private readonly DAL.user_ordergoods dal = new DAL.user_ordergoods();
        /// <summary>
        /// 无参构造函数
        /// </summary>
        ...
        /// 得到一个对象实体（或者null）
        /// </summary>
        public Model.user_ordergoods GetModel(int ID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.user_ordergoods DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                {
                    model.order_id = Convert.ToInt32(row["order_id"]);
                }
                if (row["goods_group"] != null )//&& row["goods_group"].ToString() != ""
                {
                    model.goods_group = Convert.ToInt32(row["goods_group"]);
                }
                if (row["goods_id"] != null )//&& row["goods_id"].ToString() != ""
                {
                    model.goods_id = Convert.ToInt32(row["goods_id"]);
                }
                if (row["goods_title"] != null )//&& row["goods_title"].ToString() != ""
                {
                    model.goods_title = Convert.ToString(row["goods_title"]);
                }
                ...
                {
                    model.goods_stock = Convert.ToInt32(row["goods_stock"]);
                }
        ...
        /// <param name="dt"></param>
        /// <returns></returns>      
		public List<Model.user_ordergoods> DataTableToList(DataTable dt)
		{
			List<Model.user_ordergoods> modelList = new List<Model.user_ordergoods>();
            if (dt != null)
            {
				Model.user_ordergoods model;
        ...
        /// 获得数据列表List<T>
        /// </summary>
        public List<Model.user_ordergoods> GetModelList(string whereStr)
        {
        ...
        /// <returns></returns>
        public Model.user_ordergoods GetModel(string whereStr)
        {
            DataTable dt = dal.GetList(whereStr).Tables[0];
            Model.user_ordergoods model = dt.Rows.Count > 0 ? DataRowToModel(dt.Rows[0]) : null;
            return model;
        }
        ...
        /// <returns></returns>
        public List<Model.user_ordergoods> GetModelList(int Top, string whereStr, string filedOrder)
        {
        ...
        /// <param name="filedOrder">排序字段Order By + filedOrder (不可为空)</param>
        /// <returns></returns>
        public Model.user_ordergoods GetModel(int Top, string whereStr, string filedOrder)
        {
        ...
        /// 分页获取数据列表list<T> 1
        /// </summary>
        public List<Model.user_ordergoods> GetModelListByPage(string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
        ...
        /// 分页获取数据列表list<T> 2
        /// </summary>
        public List<Model.user_ordergoods> GetModelListByPage(string DIY_FILED, string whereStr, string orderby, int pageIndex, int pageSize)
        {
            int startIndex = (pageIndex - 1) * pageSize + 1;
            int endIndex = pageIndex * pageSize;
